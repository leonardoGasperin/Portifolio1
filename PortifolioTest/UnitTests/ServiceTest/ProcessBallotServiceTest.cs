...
public class ProcessBallotServiceTest
{
    ...
    public void GetWinnerTest()
    {
        var candidates = CandidateFactory
            .GetMultipleCandidates(7)
            .Select(
                e => new CandidatesViewModel() { Name = e.Name, VoteQuantity = e.VoteQuantity, }
            )
            .ToList();
        var expectedWinner =
            candidates.First(e => e.VoteQuantity == candidates.Max(l => l.VoteQuantity))
            ?? new() { Name = "ERROR" };
    ...
    public void ShowWinnerTest()
    {
        ...
        var expectedConsoleLine =
            $"O candidato {expectedWinner.Name} venceu a eleição com {expectedWinner.VoteQuantity} votos.";
